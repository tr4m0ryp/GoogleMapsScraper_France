...
# Load France's borders from a GeoJSON file
GEOJSON_FILE_PATH = 'path_to_france_geojson.geojson'  # Update with the actual path

console.log("[bold blue]Starting the process for France...[/bold blue]")
...
if not os.path.exists(GEOJSON_FILE_PATH):
    ...
    try:
        ...
        FRANCE_BORDERS = shape(geojson_data['features'][0]['geometry'])
...
def main():
    ...
    coordinates = generate_grid_coordinates(FRANCE_BORDERS, NUM_GRIDS)
    if not coordinates:
    ...
    if all_data:
        filename = f"{query}_in_France.xlsx"
